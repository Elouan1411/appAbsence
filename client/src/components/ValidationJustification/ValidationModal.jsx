...
export default function ValidationModal({ isOpen, onClose, title, idAbsence }) {
    ...
    async function handleFetchJustification() {
        try {
            setLoading(true);
            const response = await fetch("http://localhost:3000/justification/admin/" + idAbsence, {
                method: "GET",
                credentials: "include",
            });
                ...
                <div className="modal-overlay" onClick={onClose}>
                    ...
                    >
                        ...
                        <div className="modal-body">
                            <div className="body-header">{data != null ? `${data.numeroEtudiant} - ${data.nom} ${data.prenom}` : ""}</div>
                            <div className="body-content">
                                <PDFDocument file={file} />
                            </div>
                            <div className="modal-buttons">
                                <Button className="validate-button" onClick={console.log("Valider")}>
                                    Valider
                                </Button>
                                <Button className="refuse-button" onClick={console.log("Refuser")}>
                                    Refuser
                                </Button>
                            </div>
                        </div>
